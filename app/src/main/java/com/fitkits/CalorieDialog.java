            ...
            public void onClick(View view) {
                if(calories.getText().toString().length()>0) {
                    if(noofmeal.getSelectedItemPosition()!=0) {
                        ...
                        Toast.makeText(getActivity(),"Please select the number of meals.",Toast.LENGTH_SHORT).show();
                    }
                }
                else{
                    Toast.makeText(getActivity(),"Please enter the calorie goal.",Toast.LENGTH_SHORT).show();
                }
            }
        });
